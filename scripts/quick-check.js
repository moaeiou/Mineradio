...
function runPlaybackSourceFallbackTransactionCheck() {
  ...
  const testFile = path.join(
    appRoot,
    "tests",
    "playback-source-fallback-transaction.test.js",
  );
  const result = spawnSync(process.execPath, [testFile], {
    cwd: appRoot,
    encoding: "utf8",
  ...
  if (result.status !== 0) {
    ...
    process.stderr.write(result.stderr || "");
    fail(
      `playback source fallback transaction regression failed: ${rel(testFile)}`,
    );
  }
  process.stdout.write(result.stdout || "");
}
...
function runWallpaperEngineIdleDisposeRegressionCheck() {
  ...
  const testFile = path.join(
    appRoot,
    "tests",
    "wallpaper-engine-idle-dispose.test.js",
  );
  const result = spawnSync(process.execPath, [testFile], {
    cwd: appRoot,
    encoding: "utf8",
...
function checkPersistentCacheStorageGuard() {
  ...
  if (
    ...
    !/const STABLE_USER_DATA_PATH = STARTUP_QA_USER_DATA_PATH \|\| path\.join\(app\.getPath\('appData'\), APP_NAME\)/.test(
      mainText,
    ) ||
  ...
  if (
    ...
    /cache-storage-updates-(?:path|size)/.test(htmlText + cacheUiText)
  ) {
    fail(
...
function checkExternalUpdatePageBridgeGuard() {
  ...
  const updateUiText = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
      "modules",
      "08-account",
      "00-update-preview.js",
    ),
    "utf8",
  );
  const bridgeText = mainText + "\n" + preloadText;
  if (
    !/ipcMain\.handle\('mineradio-open-update-page', async \(event, value\) =>/.test(
      mainText,
    ) ||
    ...
    !/openUpdatePage: \(url\) => ipcRenderer\.invoke\('mineradio-open-update-page'/.test(
      preloadText,
    )
  ) {
    fail(
      "desktop update bridge must open only bounded HTTPS pages from the trusted main document",
    );
  }
  if (
    /mineradio-open-update-installer|openUpdateInstaller|getUpdateDownloadDir|MINERADIO_UPDATE_DIR/.test(
      bridgeText,
    )
  ) {
    fail(
      "desktop update bridge must not expose the removed local installer or update-cache path",
    );
  }
  if (
    ...
    fail(
      "updates must resolve to an external download page and keep legacy local routes disabled",
    );
  }
  if (
    /startUpdateDownloadJob|startUpdatePatchJob|updateDownloadJobs|UPDATE_DOWNLOAD_DIR|pickPatchAsset/.test(
      serverText,
    ) ||
    /\/api\/update\/(?:download|patch)|openUpdateInstaller|快速补丁/.test(
      updateUiText,
    )
  ) {
    fail(
      "local installer download and quick-patch workers must remain removed",
    );
  }
  console.log(
    "[OK] Updates use a trusted HTTPS-only external-page bridge with no local installer path.",
  );
}
...
async function checkProviderFallbackTerminalStateGuard() {
  ...
  if (
    ...
    !/SOURCE_FALLBACK_RECOVERY_TIMEOUT_MS\s*=\s*20000/.test(fallbackText) ||
    !/function awaitSourceFallbackBudget/.test(fallbackText) ||
    (playbackText.match(/timeoutMs:\s*9000/g) || []).length < 6 ||
    (playbackText.match(/timeoutMs:\s*14000/g) || []).length < 2 ||
    (playbackText.match(/timeoutMs:\s*15000/g) || []).length < 2
  ) {
    fail(
  ...
  if (
    ...
    fail(
      "automatic recovery must use one finite transaction with queue/provider dedupe and watchdog invalidation",
    );
  }
  console.log(
...
function checkQQVipStatusSyncGuard() {
  ...
  const serverText = fs.readFileSync(path.join(appRoot, "server.js"), "utf8");
  const vipModuleText = fs.readFileSync(
    path.join(appRoot, "qq-vip-api.js"),
    "utf8",
  );
  const mainText = fs.readFileSync(
    path.join(appRoot, "desktop", "main.js"),
  ...
  if (
    ...
    !/decision:\s*'unknown'/.test(vipModuleText) ||
    !/resolveQQVipFromProbes/.test(vipModuleText)
  ) {
    fail(
  ...
  if (
    !/Promise\.allSettled\(probes\.map/.test(vipModuleText) ||
    !/qqVipSessionCacheKey/.test(serverText) ||
    /qqVipInfoCache\.get\(uin\)/.test(serverText)
  ) {
    fail(
      "QQ VIP probes must all settle and cache results by the current account session fingerprint",
    );
  }
  if (
    /vipEvidence:\s*playbackVipEvidence/.test(serverText) ||
    /member-track-playback/.test(serverText) ||
    !/function qqPlaybackShowsMemberAccess[\s\S]{0,260}return false;/.test(
      loginStatusText,
    )
  ) {
    fail(
      "QQ song hints and successful playback must not be promoted into persistent account VIP evidence",
    );
  }
  if (
  ...
  if (
    ...
    fail(
      "QQ frontend must keep last-known-good membership on transient failures and show unknown membership as pending",
    );
  }
  if (
  ...
  if (
    !/cookieIsExpired/.test(mainText) ||
    !/qqLoginCookieCandidateScore/.test(mainText) ||
    ...
    fail(
      "QQ cookie selection and end-to-end playback timeout budgets must be deterministic and aligned",
    );
  }
  if (
...
function checkPlaybackResumeRecoveryGuard() {
  ...
  if (
    ...
    !/__mineradioTrackSwitchToken\) !== Number\(trackSwitchToken\)/.test(
      progressText,
    ) ||
    !/playbackMediaMatchesCurrentQueueItem\(audioEl\)/.test(progressText) ||
    !/schedulePlaybackStallRecovery\(name,[\s\S]{0,260}ownerQueueItemKey/.test(
      progressText,
    ) ||
...
function checkSonicTopographyPresetGuard() {
  ...
  const sonicWorkshopText = fs.readFileSync(
    path.join(appRoot, "public", "sonic-workshop-preset.js"),
    "utf8",
  );
  const sonicWorkshopBridgeText = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "vendor",
      "sonic-workshop",
      "mineradio-bridge.html",
    ),
    "utf8",
  );
  const paletteText = fs.readFileSync(
  ...
  const accentControlText = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
  ...
  const colorLabText = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
  ...
  if (
    !/MAX_VISUAL_PRESET_INDEX = 8/.test(coreText) ||
    !/SONIC_PRESET_INDEX = 7/.test(coreText) ||
    !/SONIC_WORKSHOP_PRESET_INDEX = 8/.test(coreText) ||
    !/MAX_VISUAL_PRESET_INDEX/.test(runtimeText + persistenceText)
  ) {
    fail(
      "Sonic preset 7 and Workshop derivative preset 8 must survive autosave and startup restore clamps",
    );
  }
  if (
  ...
  if (
    ...
    !archiveText.includes(
      '音域回响 <span class="pc-name-en">Wallpaper Engine</span>',
    ) ||
    ...
    fail(
      "Sonic preset cards must preserve their English subtitles and Ajin/CmzYa author credits",
    );
  }
  [
    path.join(
    ...
    if (!fs.existsSync(vendorFile))
      fail(
        "Sonic Workshop derivative preset is missing packaged Wallpaper Engine asset: " +
          path.relative(appRoot, vendorFile),
      );
  });
  if (
    !/sonic-workshop-preset\.js/.test(loaderText) ||
    !/BRIDGE_SRC = 'vendor\/sonic-workshop\/mineradio-bridge\.html'/.test(
      sonicWorkshopText,
    ) ||
    !/MineradioSonicWorkshop\.update/.test(mainLoopText) ||
    !/visual\.sonic-workshop/.test(mainLoopText) ||
    !/MineradioSonicWorkshop\.onPresetChange/.test(presetGridText) ||
    !/workshopPresetActive/.test(mainLoopText)
  ) {
    fail(
      "Sonic Workshop derivative preset must load, fade as preset 8, hide base particles, and update from the main loop",
    );
  }
  if (
    !/canvasAnchor\.parentNode\.insertBefore\(layer,\s*canvasAnchor\)/.test(
      sonicWorkshopText,
    ) ||
    !/layer\.setAttribute\('inert'/.test(sonicWorkshopText) ||
    !/iframe\.setAttribute\('inert'/.test(sonicWorkshopText) ||
    !/iframe\.style\.pointerEvents\s*=\s*'none'/.test(sonicWorkshopText) ||
    !/#sonic-workshop-layer,\s*#sonic-workshop-layer \*[\s\S]{0,120}pointer-events:\s*none !important/.test(
      fs.readFileSync(path.join(appRoot, "public", "css", "index.css"), "utf8"),
    )
  ) {
    fail(
      "Sonic Workshop iframe layer must remain fully pointer-transparent so player buttons and window controls stay clickable",
    );
  }
  if (
    !/wallpaperRegisterAudioListener/.test(sonicWorkshopBridgeText) ||
    ...
    !/pulseCooldown:\s*0/.test(sonicWorkshopBridgeText) ||
    ...
    !/autoRotateSpeed:\s*7/.test(sonicWorkshopBridgeText) ||
    !/cameraAngleX:\s*150/.test(sonicWorkshopBridgeText) ||
    !/cameraAngleY:\s*30/.test(sonicWorkshopBridgeText) ||
    !/showPlayerController:\s*false/.test(sonicWorkshopBridgeText)
  ) {
    fail(
      "Sonic Workshop bridge must preserve the requested Wallpaper Engine default parameters and receive Mineradio audio/media",
    );
  }
  const sonicWorkshopVendorText = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "vendor",
  ...
  if (
    !/mineradioCustomTheme/.test(sonicWorkshopBridgeText) ||
    !/mineradioCustomTheme/.test(sonicWorkshopText) ||
    !/function workshopCustomThemeForColor/.test(sonicWorkshopText) ||
    !/function workshopPaletteHexesFromCover/.test(sonicWorkshopText) ||
    !/function workshopCustomThemeForPalette/.test(sonicWorkshopText) ||
    !/function workshopCustomThemeForRegions/.test(sonicWorkshopText) ||
    !/function workshopRegionsFromFx/.test(sonicWorkshopText) ||
    !/function applyWorkshopThemeTransition/.test(sonicWorkshopText) ||
    !/WORKSHOP_THEME_TRANSITION_MS\s*=\s*1280/.test(sonicWorkshopText) ||
    !/function applyThemeTransition/.test(sonicWorkshopBridgeText) ||
    !/THEME_TRANSITION_STEP_MS\s*=\s*33/.test(sonicWorkshopBridgeText) ||
    /scheduleWorkshopThemeTransition\(\);/.test(sonicWorkshopText) ||
    !/__mineradioPaletteHexes/.test(sonicWorkshopText) ||
    !/rawWarm/.test(paletteText + sonicWorkshopText) ||
    !/rawCool/.test(paletteText + sonicWorkshopText) ||
    !/rawAreaPrimary/.test(
      paletteText + sonicWorkshopText + accentControlText,
    ) ||
    !/sonicWorkshopColors/.test(paletteText + sonicWorkshopText) ||
    !/coverSourceKey/.test(paletteText + accentControlText) ||
    !/function ensureSonicWorkshopCoverPaletteForUi/.test(accentControlText) ||
    !/function sonicWorkshopCurrentCoverDomSource/.test(accentControlText) ||
    !/function buildSonicWorkshopUiPaletteFromCanvas/.test(accentControlText) ||
    !/function sonicWorkshopUiPaletteForKey/.test(accentControlText) ||
    !/sonicWorkshopCoverUiSample\.palette/.test(accentControlText) ||
    !/coverPickerCanvas/.test(accentControlText) ||
    !/coverProxySrc/.test(accentControlText) ||
    !/coverColors/.test(paletteText + sonicWorkshopText) ||
    !/sonic-workshop-cool-picker/.test(indexText + fxBindText) ||
    !/sonic-workshop-peak-picker/.test(indexText + fxBindText) ||
    !/setSonicWorkshopRegionColorFromPicker/.test(
      fxBindText + sonicWorkshopText + accentControlText,
    ) ||
    !/function sonicRawPaletteHex/.test(accentControlText) ||
    /function sonicWorkshopCoverHex[\s\S]{0,700}sonicPaletteHex/.test(
      accentControlText,
    ) ||
    !/uCoolCore:\s*cool/.test(sonicWorkshopText) ||
    !/uRippleColor:\s*ripple/.test(sonicWorkshopText) ||
    !/MRt\(We\.mineradioCustomTheme\.value\)/.test(sonicWorkshopVendorText) ||
    !/Be\(function\(Mr\)\{return Mr===0\?1e-6:0\}\)/.test(
      sonicWorkshopVendorText,
    )
  ) {
    fail(
      "Sonic Workshop cover/custom colors must feed the real vendor terrain theme instead of only recoloring the UI controls",
    );
  }
  if (
    !/colorLabState\.picker\)\s*colorLabState\.picker\.value\s*=\s*hex/.test(
      colorLabText,
    ) ||
    !/id === 'sonic-workshop-cover-picker'[\s\S]{0,180}\^sonic-workshop-/.test(
      colorLabText,
    ) ||
    !/pointerdown[\s\S]{0,180}updateColorLabFromSv\(e\)/.test(fxBindText) ||
    !/function sonicWorkshopRegionControl\(id\)\s*\{[\s\S]{0,120}typeof id === 'object' && id\.id/.test(
      accentControlText,
    ) ||
    !/function pushSonicWorkshopColorChange/.test(accentControlText) ||
    !/function setSonicWorkshopThemeFromPicker[\s\S]{0,520}SONIC_WORKSHOP_COLOR_CONTROLS\.forEach/.test(
      accentControlText,
    ) ||
    !/pushSonicWorkshopColorChange\(item\.colorKey\)/.test(accentControlText)
  ) {
    fail(
      "Sonic Workshop color lab changes must commit into fx state, UI swatches, saved settings, and the live iframe theme",
    );
  }
  if (
  ...
  if (
    !/global\.frequencyData/.test(sonicWorkshopText) ||
    /sonicAudioMonitorState/.test(sonicWorkshopText) ||
    /MineradioSonicWorkshop\.update\([\s\S]{0,260}audio:\s*sonicAudioFrame/.test(
      mainLoopText,
    )
  ) {
    fail(
      "Sonic Workshop derivative must use the local player analyser bridge instead of the original Sonic realtime spectrum frame",
    );
  }
  if (
    !/WORKSHOP_AUDIO_TARGET_MAX_SAMPLE\s*=\s*0\.52/.test(sonicWorkshopText) ||
    !/WORKSHOP_AUDIO_GAMMA\s*=\s*1\.55/.test(sonicWorkshopText) ||
    !/WORKSHOP_AUDIO_MIN_FLOOR\s*=\s*0\.035/.test(sonicWorkshopText) ||
    !/function workshopAudioFrameStats/.test(sonicWorkshopText) ||
    !/function shapeWorkshopAudioValue/.test(sonicWorkshopText) ||
    !/sonicWorkshopInputGain/.test(sonicWorkshopText) ||
    /Math\.pow\(clamp01\(value\),\s*0\.68\)/.test(sonicWorkshopText) ||
    /1\.05\s*\+\s*energy\s*\*\s*0\.34/.test(sonicWorkshopText)
  ) {
    fail(
      "Sonic Workshop bridge must keep Wallpaper-like dark-field audio shaping instead of overdriving the terrain",
    );
  }
  if (
    ...
    fail(
      "Sonic Workshop derivative must use packaged assets and Mineradio analyser data instead of runtime capture or the Steam workshop path",
    );
  }
  if (
...
function runMainStartupRecoveryCheck() {
  ...
  const qaUserDataParent = path.join(
    process.env.TEMP || appData,
    "mineradio-startup-qa",
  );
