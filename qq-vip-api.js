...
function qqVipObjectExpiry(obj) {
  const values = [];
  if (!objectHasDirectMembershipKey(obj))
    return { present: false, expired: false, expiresAt: 0 };
...
function assessQQVipObject(obj, context, now) {
  ...
  const hasPairedMembershipSignal =
    hasDirectMembershipKey ||
    !!(
      context &&
      context.membership &&
      keys.some((key) => {
        const normalized = canonicalQQVipKey(key);
        return (
          MEMBERSHIP_STATUS_KEYS.has(normalized) ||
          VIP_MEMBERSHIP_STATUS_KEYS.has(normalized) ||
          SVIP_MEMBERSHIP_STATUS_KEYS.has(normalized) ||
  ...
  keys.forEach((key) => {
    ...
    if (hasPairedMembershipSignal && isQQMembershipExpiryKey(normalized)) {
      ...
      if (expiry > 0) {
        const tier = qqMembershipExpiryTier(
          normalized,
          context,
          hasDirectVipSignal,
          hasDirectSvipSignal,
...
function fallbackQQVipStatus(fallback) {
  ...
  return {
    ...
    vipExpiresAt: !isSvip
      ? Number(fallback.expiresAt || fallback.expireAt || 0) || 0
      : 0,
    svipExpiresAt: isSvip
      ? Number(fallback.expiresAt || fallback.expireAt || 0) || 0
      : 0,
...
function normalizeQQVipPayload(payload, fallback, options) {
  ...
  const vipExpiries = vipPositive
    .map((item) => Number(item.vipExpiresAt) || 0)
    .filter((value) => value > now);
  const svipExpiries = svipPositive
    .map((item) => Number(item.svipExpiresAt) || 0)
    .filter((value) => value > now);
  ...
  const vipType = Math.max(
    1,
    ...vipPositive.map((item) => Number(item.vipType) || 0),
  );
  const svipType = isSvip
    ? Math.max(1, ...svipPositive.map((item) => Number(item.svipType) || 0))
    : 0;
  return {
    vipType,
    svipType,
...
function combineQQVipResults(results) {
  ...
  if (positives.length) {
    ...
    const vipExpiries = positives
      .map(
        (item) =>
          Number(item.vipExpiresAt) ||
          (!item.isSvip && item.vipLevel !== "svip"
...
async function resolveQQVipFromProbes(probes, requestProbe) {
  ...
  const completedProbeCount = values.filter(
    (item) => item.probeSuccessful,
  ).length;
  const negativeQuorum = Math.min(2, Math.max(1, probeCount));
  const authoritativeNegative =
    !positiveValues.length &&
    authoritativeNegativeValues.length >= negativeQuorum;
  ...
  combined.accountMatchedProbeCount = values.filter(
    (item) => item.accountMatched && item.probeSuccessful,
  ).length;
...
function preserveQQVipStalePositive(cachedEntry, nextStatus, options) {
  ...
  const graceMs = Math.max(
    0,
    Number(options.graceMs) || QQ_VIP_STALE_POSITIVE_GRACE_MS,
  );
  const cachedStatus =
    cachedEntry && cachedEntry.value ? cachedEntry.value : cachedEntry;
  if (
    !cachedStatus ||
    !cachedStatus.isVip ||
    !cachedStatus.resolved ||
    cachedStatus.membershipKnown !== true
  ) {
    return nextStatus || null;
  }
  if (nextStatus && nextStatus.isVip && nextStatus.resolved) return nextStatus;
  if (nextStatus && nextStatus.authoritativeNegative === true)
    return nextStatus;
  const nextProbeIncomplete =
    !nextStatus ||
  ...
  if (entitlementExpiresAt > 0 && entitlementExpiresAt <= now)
    return nextStatus || null;
  const cacheExpiresAt = Number(cachedEntry && cachedEntry.expiresAt) || now;
  const staleUntil =
    Number(cachedEntry && cachedEntry.staleUntil) ||
    Math.min(
