...
function supportedAudioPath(value) {
  const resolved = normalizedAbsoluteFilePath(value);
  return resolved && AUDIO_MIME.has(path.extname(resolved).toLowerCase())
    ? resolved
    : "";
}
...
function localFileId(filePath) {
  return crypto
    .createHash("sha256")
    .update(normalizedPathIdentity(filePath))
    .digest("hex")
    .slice(0, 24);
}
...
function coverWithinBudget(data, mime) {
  if (!Buffer.isBuffer(data) || !data.length || data.length > MAX_COVER_BYTES)
    return false;
  ...
  for (const item of lyrics) {
    ...
    if (syncText.length) {
      const lines = syncText
        ...
        .map(
          (line) =>
            `${formatLrcTimestamp(line.timestamp)}${String(line.text || "").trim()}`,
        );
...
function normalizeImportEntries(input) {
  const entries = [];
  const seen = new Set();
  for (const item of Array.isArray(input)
    ? input.slice(0, MAX_IMPORT_FILES)
    : []) {
...
async function mapWithConcurrency(items, limit, worker) {
  ...
  let cursor = 0;
  const runners = new Array(Math.min(Math.max(1, limit), items.length))
    .fill(null)
    .map(async () => {
...
async function buildLrcSidecarIndex(entries) {
  const directories = Array.from(
    new Set(entries.map((entry) => path.dirname(entry.path))),
  );
  const maps = new Map();
...
class LocalMusicLibrary {
  constructor(options = {}) {
    this.userDataPath = path.resolve(
      String(options.userDataPath || process.cwd()),
    );
    this.libraryDirectory = path.join(
      this.userDataPath,
      LOCAL_LIBRARY_DIRECTORY,
    );
    this.coverDirectory = path.join(
      this.libraryDirectory,
      LOCAL_COVER_DIRECTORY,
    );
    this.indexPath = path.join(this.userDataPath, LOCAL_LIBRARY_FILE);
    this.parseMetadata =
      typeof options.parseMetadata === "function"
  ...
  loadIndex() {
    try {
      ...
      if (
        !stat.isFile() ||
        stat.size <= 0 ||
        stat.size > MAX_LIBRARY_INDEX_BYTES
      )
        return;
      const parsed = JSON.parse(fs.readFileSync(this.indexPath, "utf8"));
      if (
        !parsed ||
        parsed.version !== LOCAL_LIBRARY_VERSION ||
        !Array.isArray(parsed.records)
      )
        return;
      if (/^[a-f0-9]{48}$/i.test(String(parsed.mediaToken || "")))
        this.mediaToken = String(parsed.mediaToken).toLowerCase();
      ...
      for (const source of parsed.records.slice(0, MAX_IMPORT_FILES)) {
        ...
        if (!coverPath || !isPathInside(this.coverDirectory, coverPath))
          coverPath = "";
        const record = {
          id,
          audioPath,
  ...
  serializeRecord(record) {
    ...
    return {
      ...
      cover: coverAvailable
        ? localMediaUrl("cover", record.id, record.revision, this.mediaToken)
        : "",
  ...
  async stageCover(id, picture, previous) {
    ...
    const temporary = path.join(
      this.coverDirectory,
      `.${id}-${digest}.${process.pid}.${Date.now()}.stage`,
    );
  ...
  async parseEntry(entry, sidecarDirectories) {
    ...
    try {
      ...
      metadataError = String(
        (error && error.message) || error || "METADATA_PARSE_FAILED",
      ).slice(0, 500);
    }
    ...
    const artists = Array.isArray(common.artists)
      ...
      : "";
    const picture =
      Array.isArray(common.picture) && common.picture.length
        ? common.picture[0]
        : null;
    ...
    const directoryLookup = sidecarDirectories.get(
      normalizedPathIdentity(path.dirname(entry.path)),
    );
    const sidecarPath =
      directoryLookup && directoryLookup.get(fallbackTitle.toLowerCase());
    let lyric = "";
    let lyricSource = "";
    if (sidecarPath) {
      try {
        ...
        if (
          lyricStat.isFile() &&
          lyricStat.size > 0 &&
          lyricStat.size <= MAX_LYRIC_BYTES
        ) {
          lyric = decodeLyricBuffer(
            await fs.promises.readFile(sidecarPath),
          ).slice(0, MAX_LYRIC_BYTES);
    ...
    return {
      record: {
        ...
        name: cleanText(
          common.title,
          metadataError && previous ? previous.name : fallbackTitle,
          1000,
        ),
        artist: cleanText(
          common.artist || artists,
          metadataError && previous ? previous.artist : "本地文件",
          1000,
        ),
        album: cleanText(
          common.album,
          metadataError && previous ? previous.album : fallbackAlbum,
          1000,
        ),
        duration: Math.max(
          0,
          Number(format.duration) ||
            (metadataError && previous ? Number(previous.duration) : 0) ||
            0,
        ),
    ...
    const operation = async () => {
      ...
      for (const result of parsed) {
        ...
        if (result.metadataError)
          metadataWarnings.push({
            name: path.basename(record.audioPath),
            error: result.metadataError,
          });
        if (result.coverWarning)
          metadataWarnings.push({
            name: path.basename(record.audioPath),
            error: result.coverWarning,
          });
        if (result.stagedCoverPath) {
          stagedCovers.push({
            stagedPath: result.stagedCoverPath,
            targetPath: record.coverPath,
          });
        }
        if (
          result.previousCoverPath &&
          (!record.coverPath ||
            path.resolve(result.previousCoverPath) !==
              path.resolve(record.coverPath))
        ) {
      ...
      if (!nextOrder.length) {
        ...
        return {
          ...
          failures,
          metadataWarnings,
          error: "LOCAL_IMPORT_FAILED",
        };
      }
      const removedRecords = replace
        ? this.order
            .filter((id) => !nextRecords.has(id))
  ...
  recordForRequest(requestUrl) {
    try {
      ...
      if (
        !kind ||
        !/^[a-f0-9]{24}$/.test(id) ||
        url.searchParams.get("cap") !== this.mediaToken
      )
        return null;
      const record = this.records.get(id);
      ...
      if (
        kind === "cover" &&
        (!isPathInside(this.coverDirectory, filePath) ||
          !COVER_MIME_BY_EXTENSION.has(path.extname(filePath).toLowerCase()))
      )
        return null;
  ...
  async mediaResponse(request) {
    ...
    const contentType =
      target.kind === "audio"
        ...
        : target.record.coverMime ||
          COVER_MIME_BY_EXTENSION.get(extension) ||
  ...
  async installProtocol(protocol) {
    ...
    await protocol.handle(LOCAL_MUSIC_SCHEME, (request) =>
