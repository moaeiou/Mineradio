...
function readConfig(file) {
  try {
    ...
    console.warn(
      "[QishuiQrLogin] ignored invalid config:",
...
function hasLoginCookie(cookie) {
  return /(?:^|;\s*)(?:sessionid|sessionid_ss|sid_guard|sid_tt)=/i.test(
    String(cookie || ""),
  );
}
...
function createQishuiQrLoginBridge(options) {
  ...
  const configFile =
    options.configFile ||
    process.env.QISHUI_QR_CONFIG_FILE ||
    DEFAULT_CONFIG_FILE;
  let config = {
    ...readConfig(configFile),
    ...(options.initialConfig && typeof options.initialConfig === "object"
