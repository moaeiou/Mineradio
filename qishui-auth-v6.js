...
class QishuiAuthRuntime {
  ...
  _installSessionHooks() {
    ...
    const mfaUrls = [
      "https://api.qishui.com/passport/*",
      "https://verify.zijieapi.com/passport/*",
    ];
    authSession.webRequest.onCompleted({ urls: mfaUrls }, (details) => {
      this.mfaNetworkLog.push({
        at: Date.now(),
        method: details.method,
        endpoint: this.safeEndpoint(details.url),
  ...
  async _initialize() {
    ...
    this.browserInfo = await this.window.webContents.executeJavaScript(
      "window.__qishuiBrowserInfo()",
      true,
    );
  ...
  async request(method, pathname, params, data) {
    ...
    if (!response || response.status < 200 || response.status >= 400) {
      throw new Error(
        `汽水登录接口 HTTP ${(response && response.status) || 0}: ${String((response && response.body) || "").slice(0, 300)}`,
      );
    }
    let envelope;
    try {
    ...
    const signedUrl = String(
      (this.lastPassportRequest && this.lastPassportRequest.url) || "",
    );
    let signedQuery;
    try {
  ...
  async getQrCode() {
    ...
    if (envelope.message !== "success" || Number(data.error_code) !== 0) {
      throw new Error(
        `二维码生成失败：code=${data.error_code} ${data.description || envelope.message || ""}`,
      );
    }
    const scanUrl = officialScanUrl(
      data.qrcode_index_url,
  ...
  async checkQrConnect(token) {
    ...
    let envelope = await this.request(
      "POST",
    ...
    if (Number(data.error_code) === 2046) {
      ...
      if (!verified || verified.status !== true) {
        const error = new Error(
          (verified && verified.message) || "二次验证未完成",
        );
  ...
  async clear() {
    if (this.authSession) {
      ...
      try {
        await this.authSession.flushStorageData();
      } catch (_) {}
    }
    ...
    this.msToken = "";
    if (this.assetServer)
      await new Promise((resolve) => this.assetServer.close(resolve));
    this.assetServer = null;
    this.assetBase = "";
    this.assetToken = crypto.randomBytes(18).toString("hex");
    this.destroying = false;
