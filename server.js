...
const APP_VERSION =
  process.env.MINERADIO_VERSION || APP_PACKAGE.version || "2.1.0";
...
function extractReleaseNotes(body) {
  ...
  String(body || "")
    ...
    .forEach((line) => {
      ...
      if (
        /^(下载|网盘|夸克盘|百度(?:云|网盘)|蓝奏(?:云|网盘)|安装包)/i.test(text)
      )
        return;
...
function normalizeUpdateDownloadPages(values, fallbackLabel) {
  ...
  source.forEach((value, index) => {
    ...
    const url = safeExternalUpdateUrl(
      item.url || item.href || item.downloadPageUrl || item.externalUrl || "",
    );
    if (!url || seen.has(url)) return;
    const label =
      cleanReleaseLine(
        item.label || item.name || fallbackLabel || `下载线路 ${index + 1}`,
      )
        .replace(/[<>|]/g, "")
        .slice(0, 24)
        .trim() || `下载线路 ${index + 1}`;
...
function extractReleaseDownloadPages(body) {
  ...
  const visiblePattern =
    /^\s*[-*]?\s*(夸克盘|百度(?:云|网盘)|蓝奏(?:云|网盘)|网盘|下载(?:地址|页面|链接)?)\s*[:：]\s*(?:\[[^\]]*]\()?(https:\/\/[^\s<>)\]]+)/gim;
  let visible = null;
...
function normalizeManifestUpdateInfo(data) {
  ...
  const htmlUrl = safeExternalUpdateUrl(
    release.htmlUrl || release.html_url || data.htmlUrl || "",
  );
...
function normalizeQQCookieInput(cookieText) {
  ...
  if ((obj.wxopenid || Number(obj.login_type) === 2) && obj.wxuin)
    obj.uin = obj.wxuin;
  ...
  if (
    ...
    sendJSON(
      res,
      {
        ...
        message:
          "Mineradio 已停用客户端本地下载与快速补丁，请使用外部下载页面。",
      },
      410,
    );
    return;
