...
(function (global) {
  ...
  function currentSong() {
    if (
      Array.isArray(global.playQueue) &&
      global.currentIdx >= 0 &&
      global.currentIdx < global.playQueue.length
    )
      return global.playQueue[global.currentIdx];
    if (
      Array.isArray(global.playlist) &&
      global.currentIdx >= 0 &&
      global.currentIdx < global.playlist.length
    )
      return global.playlist[global.currentIdx];
    return null;
  }
  ...
  function workshopCoverHex(role) {
    var pal =
      (global.stageLyrics &&
        (global.stageLyrics.coverPalette || global.stageLyrics.palette)) ||
      {};
  ...
  function workshopPaletteHexesFromCover() {
    var pal =
      ...
      {};
    var values = [];
    if (Array.isArray(pal.sonicWorkshopColors))
      values = values.concat(pal.sonicWorkshopColors);
    ...
    values.forEach(function (value) {
      ...
      for (var i = 0; i < out.length; i++) {
        if (out[i].toLowerCase() === key || colorDistance(out[i], hex) < 0.035)
          return;
      }
      out.push(hex);
    });
  ...
  function workshopPickPaletteHex(hexes, kind, fallback, avoidHex) {
    ...
    (hexes && hexes.length ? hexes : [fallback || "#cb6c89"]).forEach(
      function (hex) {
        var info = workshopHexInfo(hex);
        var h = info.hsl.h;
        var score = 0;
        if (kind === "dark") score = (1 - info.lum) * 1.2 + info.chroma * 0.34;
        else if (kind === "light") score = info.lum * 1.18 + info.chroma * 0.34;
        else if (kind === "warm")
          score =
            Math.max(
              workshopHueBandScore(h, 0.06, 0.22),
              workshopHueBandScore(h, 0.98, 0.16),
            ) *
              1.05 +
            info.chroma * 0.62 +
            info.lum * 0.1;
          ...
          score =
            workshopHueBandScore(h, 0.56, 0.3) * 1.08 +
            info.chroma * 0.58 +
            info.lum * 0.1;
        else if (kind === "accent")
          score =
            info.chroma * 0.95 +
            Math.min(0.42, Math.abs(info.lum - 0.5)) * 0.2 +
            (avoidHex ? colorDistance(info.hex, avoidHex) * 0.36 : 0);
        else
          score = info.chroma * 0.88 + (0.5 - Math.abs(info.lum - 0.56)) * 0.34;
        if (score > bestScore) {
          best = info.hex;
          bestScore = score;
        }
  ...
  function normalizeWorkshopTheme(theme) {
    ...
    return /^(nocturnal|coral-mirage|ocean-deep|arctic-aurora|cyber-forest|minimal-monochrome|neon-tokyo|golden-hour|ember-fire|crimson-sunset)$/.test(
      theme,
    )
      ? theme
      : "coral-mirage";
  ...
  function workshopCustomThemeForColor(hex) {
    ...
    var rgb = hexRgb(hex, "#cb6c89");
    var hsl = rgbToHslLocal(rgb.r, rgb.g, rgb.b);
    var peakRgb = hslToRgbLocal(
      hsl.h + 0.54,
      clamp(0.62 + hsl.s * 0.28, 0.62, 0.92),
      clamp(0.54 + hsl.l * 0.14, 0.5, 0.72),
    );
    var peak = rgbToHexLocal(peakRgb.r, peakRgb.g, peakRgb.b);
    var glow = clamp(0.62 + hsl.s * 0.2 + hsl.l * 0.1, 0.62, 0.98);
    return {
  ...
  function workshopCustomThemeForRegions(regions) {
    ...
    var base = normalizeHex(
      regions.base || mixHex("#000000", primary, 0.16),
      "#16060f",
    );
    var warm = normalizeHex(regions.warm || primary, primary);
    var cool = normalizeHex(regions.cool || "#99c4ff", "#99c4ff");
    var ripple = normalizeHex(
      regions.ripple || regions.peak || cool,
      "#f8d8ff",
    );
    ...
    var glow = clamp(
      0.62 +
        info.chroma * 0.1 +
        Math.max(coolInfo.chroma, peakInfo.chroma) * 0.08 +
        Math.max(0, info.lum - 0.38) * 0.06,
      0.62,
      0.96,
    );
    return {
  ...
  function workshopRegionHex(fx, role, colorKey, modeKey, fallback) {
    ...
    if (fx && fx[modeKey] === "custom")
      return cssColorToHex(fx[colorKey] || fallback, fallback);
  ...
  function workshopRegionsFromFx(fx) {
    ...
    return {
      primary: primary,
      base: workshopRegionHex(
        fx,
        "base",
        "sonicWorkshopBaseColor",
        "sonicWorkshopBaseColorMode",
        mixHex("#000000", primary, 0.12),
      ),
      warm: workshopRegionHex(
        fx,
        "warm",
  ...
  function workshopThemeSignature(theme) {
    ...
    return JSON.stringify(
      WORKSHOP_THEME_COLOR_KEYS.map(function (key) {
        return normalizeHex(theme[key] || "#000000", "#000000");
      }).concat([
        Number(theme.uGlowIntensity) || 1,
        normalizeHex(theme.__primaryColor || "#cb6c89", "#cb6c89"),
      ]),
    );
  }
  ...
  function cloneWorkshopTheme(theme) {
    ...
    var out = {
      ...
      __primaryColor: normalizeHex(
        theme.__primaryColor || "#cb6c89",
        "#cb6c89",
      ),
  ...
  function mixWorkshopTheme(from, to, t) {
    ...
    var out = {
      ...
      uGlowIntensity: Number(
        (
          from.uGlowIntensity +
          (to.uGlowIntensity - from.uGlowIntensity) * t
        ).toFixed(3),
      ),
    };
    WORKSHOP_THEME_COLOR_KEYS.forEach(function (key) {
  ...
  function buildMediaState() {
    ...
    var cover = "";
    if (song && typeof global.songCoverSrc === "function")
      cover = global.songCoverSrc(song, 512) || "";
    if (!cover && song && song.cover) {
      cover =
        typeof global.coverUrlWithSize === "function"
          ? global.coverUrlWithSize(song.cover, 512)
          : song.cover;
    }
    var pal =
      (global.stageLyrics &&
      ...
      {};
    var duration =
      global.audio && isFinite(global.audio.duration)
        ? Number(global.audio.duration)
        : Number(song && (song.duration || song.durationSec || 0)) || 0;
    if (duration > 10000) duration /= 1000;
    return {
      title: String((song && (song.name || song.title)) || ""),
      artist: String((song && (song.artist || song.ar || song.author)) || ""),
      thumbnail: cover || "",
      primaryColor: cssColorToHex(
      ...
      position:
        global.audio && isFinite(global.audio.currentTime)
          ? Number(global.audio.currentTime)
          : 0,
      duration: duration,
    };
  }
  ...
  function postMessage(type, payload) {
    ...
    try {
      if (
        type === "mineradio-sonic-workshop-audio" &&
        typeof frame.__mineradioApplyAudio === "function"
      ) {
      ...
      if (
        ...
        typeof frame.__mineradioApplyMedia === "function"
      ) {
      ...
      if (
        ...
        typeof frame.__mineradioApplyProperties === "function"
      ) {
    ...
    try {
      frame.postMessage(Object.assign({ type: type }, payload), "*");
    } catch (e2) {}
  }
  ...
  function ensureLayer() {
    ...
    if (!layer) {
      ...
      if (canvasAnchor && canvasAnchor.parentNode)
        canvasAnchor.parentNode.insertBefore(layer, canvasAnchor);
    ...
    if (state.themeTransitionRaf) {
      try {
        if (global.cancelAnimationFrame)
          global.cancelAnimationFrame(state.themeTransitionRaf);
  ...
  function bodyClass(active) {
    if (document && document.body)
      document.body.classList.toggle("sonic-workshop-active", !!active);
  }
  ...
  function deriveProperties(fx) {
    ...
    props.peakColorIntensity = clamp(
      fx.sonicWorkshopPeakIntensity == null
        ? props.peakColorIntensity
    ...
    props.__mineradioNearestTheme =
      fx.sonicWorkshopColorMode === "custom"
        ? customTheme
        : workshopThemeForColor(colorHex);
    return props;
  }
  ...
  function pushProperties(force) {
    ...
    if (
      !force &&
      key === state.lastPropertiesKey &&
      now - state.lastPropertiesAt < PROPERTIES_PUSH_INTERVAL_MS
    )
      return;
  ...
  function pushMedia(force) {
    ...
    if (
      !force &&
      key === state.lastMediaKey &&
      now - state.lastMediaAt < MEDIA_PUSH_INTERVAL_MS
    )
      return;
  ...
  function rawAudioArray() {
    if (global.frequencyData && global.frequencyData.length)
      return global.frequencyData;
    return null;
  }

  function frameValue(audioFrame, key) {
    return audioFrame && isFinite(audioFrame[key])
      ? Number(audioFrame[key])
      : 0;
  }
  ...
  function workshopAudioFrameStats(raw) {
    ...
    if (!len)
      return {
        mean: 0,
        max: 0,
        floor: WORKSHOP_AUDIO_MIN_FLOOR,
        peakFloor: 0.38,
        bodyGain: WORKSHOP_AUDIO_BODY_GAIN,
      };
    for (var i = 0; i < 512; i++) {
    ...
    return {
      ...
      max: max,
      floor: clamp(
        Math.max(WORKSHOP_AUDIO_MIN_FLOOR, mean * 0.48),
        WORKSHOP_AUDIO_MIN_FLOOR,
        0.2,
      ),
      peakFloor: clamp(Math.max(0.38, mean * 1.72), 0.38, 0.76),
      bodyGain: clamp(
        WORKSHOP_AUDIO_BODY_GAIN - mean * 0.1,
        0.28,
        WORKSHOP_AUDIO_BODY_GAIN,
      ),
    };
  }
  ...
  function shapeWorkshopAudioValue(value, i, stats, beat, bassDrive) {
    var floor =
      stats && isFinite(stats.floor) ? stats.floor : WORKSHOP_AUDIO_MIN_FLOOR;
    var body = Math.pow(
      clamp01((value - floor) / Math.max(0.001, 1 - floor)),
      WORKSHOP_AUDIO_GAMMA,
    );
    var peakFloor = stats && isFinite(stats.peakFloor) ? stats.peakFloor : 0.38;
    var peak = Math.pow(
      clamp01((value - peakFloor) / Math.max(0.001, 1 - peakFloor)),
      1.08,
    );
    var lowLift =
      i < 36
        ? (1 - i / 36) * (beat * WORKSHOP_AUDIO_LOW_LIFT + bassDrive * 0.022)
        : 0;
    var shaped =
      body * ((stats && stats.bodyGain) || WORKSHOP_AUDIO_BODY_GAIN) +
      peak * WORKSHOP_AUDIO_PEAK_GAIN +
      lowLift;
  ...
  function buildAudioSamples(audioFrame) {
    ...
    var inputGainValue =
      global.fx && global.fx.sonicWorkshopInputGain != null
        ? Number(global.fx.sonicWorkshopInputGain)
        : 82;
    var inputGain = clamp(inputGainValue, 40, 100) / 100;
    var beat = Math.max(
      frameValue(audioFrame, "beat"),
      frameValue(audioFrame, "kickEnvelope"),
      frameValue(audioFrame, "triggerPulse"),
    );
    var bassDrive = Math.max(
      frameValue(audioFrame, "subBass"),
    ...
    if (raw && raw.length) {
      ...
      var bands = [
        ...
        frameValue(audioFrame, "brilliance") ||
          frameValue(audioFrame, "treble"),
      ...
      for (var j = 0; j < 512; j++) {
        ...
        var v = a + (b - a) * mix;
        var fallbackLift =
          j < 36 ? beat * (1 - j / 36) * WORKSHOP_AUDIO_LOW_LIFT : 0;
        out[j] =
          clamp(
            Math.pow(clamp01(v), 1.35) * 0.3 + fallbackLift,
            0,
            WORKSHOP_AUDIO_TARGET_MAX_SAMPLE,
          ) * inputGain;
      }
    }
    if (!(global.playing && global.audio && !global.audio.paused)) {
      for (var k = 0; k < out.length; k++)
        out[k] = out[k] * WORKSHOP_AUDIO_PAUSED_GAIN;
    }
    state.samples = out;
    return out;
  }
  ...
  function pushAudio(force, audioFrame) {
    ...
    postMessage("mineradio-sonic-workshop-audio", {
      samples: buildAudioSamples(audioFrame || {}),
    });
  }
  ...
  function update(dt, ctx) {
    ...
    state.opacity +=
      ...
      clamp(1 - Math.exp(-rate * Math.max(0.001, dt || 1 / 60)), 0, 1);
  ...
  global.MineradioSonicWorkshop = {
    ...
    update: update,
    clear: clear,
    pushProperties: function (force) {
      pushProperties(force === true);
    },
