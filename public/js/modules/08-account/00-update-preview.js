...
function normalizeUpdateDownloadPages(values) {
  ...
  source.forEach(function (value, index) {
    ...
    var url = String(
      item.url || item.href || item.downloadPageUrl || item.externalUrl || "",
    ).trim();
    if (!isSafeUpdatePageUrl(url) || seen[url]) return;
    var label =
      String(item.label || item.name || "下载线路 " + (index + 1))
        .replace(/[<>|]/g, "")
        .trim()
...
function currentUpdatePageUrl(preferredIndex) {
  ...
  if (pages[index] && isSafeUpdatePageUrl(pages[index].url))
...
function applyLatestUpdateInfo(data) {
  ...
  updatePreviewState.externalUrl =
    release.externalUrl || data.externalUrl || "";
  ...
  if (
    ...
    !updatePreviewState.downloadPages.some(function (page) {
      return page.url === updatePreviewState.externalUrl;
    })
  ) {
  ...
  if (
    updatePreviewState.selectedDownloadPageIndex >=
...
function syncUpdatePreviewStateClass() {
  ...
  var selectedPage =
    downloadPages[Number(updatePreviewState.selectedDownloadPageIndex || 0)] ||
    downloadPages[0] ||
    null;
  ...
  Array.prototype.forEach.call(sourceButtons, function (sourceButton) {
    ...
    sourceButton.classList.toggle(
      "active",
      index === Number(updatePreviewState.selectedDownloadPageIndex || 0),
    );
  });
...
function animateUpdatePanelContents() {
  ...
  var sources = Array.prototype.slice.call(
    modal.querySelectorAll(".update-download-source"),
  );
...
async function startUpdatePreviewDownload(preferredIndex) {
  ...
  try {
    if (
      ...
      typeof window.desktopWindow.openUpdatePage === "function"
    ) {
      var result = await window.desktopWindow.openUpdatePage(target);
      if (!result || result.ok === false)
        throw new Error((result && result.error) || "OPEN_UPDATE_PAGE_FAILED");
    } else {
    ...
    updatePreviewState.errorReason =
      (e && e.message) || "OPEN_UPDATE_PAGE_FAILED";
    syncUpdatePreviewStateClass();
    showToast("无法打开更新页面");
  }
}
