...
(function loadMineradioIndexModules() {
  ...
  // Prefer the prebuilt single-file bundle (generated by
  ...
  try {
    ...
    // Ignore; fall back to per-module loading below.
  }
  if (
    bundleRequest.status >= 200 &&
    bundleRequest.status < 300 &&
    bundleRequest.responseText
  ) {
    injectScriptText(bundleRequest.responseText);
    return;
  }
  ...
  injectScriptText(modulePaths.map(readModule).join(""));
})();
