...
test("SVIP-only track flags are normalized and gated independently from VIP", () => {
  ...
  assert.equal(
    api.qishuiRequiredTierAllowed(restriction.requiredTier, membership("vip")),
    false,
  );
  assert.equal(
    api.qishuiRequiredTierAllowed(restriction.requiredTier, membership("svip")),
    true,
  );
});
...
test("VIP and SVIP expiry fields are evaluated independently", () => {
  ...
  assert.equal(
    activeSvipWithExpiredVip.isVip,
    true,
    "SVIP must imply the base VIP entitlement",
  );
