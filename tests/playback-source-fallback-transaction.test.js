...
function createSandbox(queue, statusOverrides) {
  ...
  const sandbox = {
    ...
    platformStatus(provider) {
      return statuses[provider] || { loggedIn: false };
    },
    ...
    clearAlbumGaplessPreload() {
      sandbox.albumGaplessClears = (sandbox.albumGaplessClears || 0) + 1;
    },
    resetCuefieldAutoMix() {
      sandbox.cuefieldClears = (sandbox.cuefieldClears || 0) + 1;
    },
    clearPlaybackResumeWatchdogs() {
      sandbox.watchdogClears = (sandbox.watchdogClears || 0) + 1;
    },
    setPlayIcon(value) {
...
async function testFiniteQueueRecovery() {
  ...
  assert.strictEqual(
    childCalls,
    2,
    "recovery may advance at most two queue entries",
  );
  assert(
    recovery && recovery.terminal,
    "the finite recovery transaction must settle",
  );
  ...
  assert.strictEqual(
    sandbox.playbackResumeRecovery.serial,
    4,
    "terminal state invalidates late media watchdogs",
  );
  assert.strictEqual(
...
async function testDuplicateSongProviderDeduplication() {
  ...
  assert.strictEqual(
    searchCalls,
    1,
    "the same song/provider pair must be searched once",
  );
  assert.strictEqual(childCalls, 1);
  assert.strictEqual(
    sandbox.playQueue[0].provider,
    "netease",
    "failed provisional source must roll back",
  );
  assert.strictEqual(
    sandbox.playQueue[1],
    duplicate,
    "duplicate queue item must not be scanned again",
  );
}
...
async function testLateAsyncCannotReviveTerminal() {
  ...
  const settled = sandbox.settleSourceFallbackTerminal(0, 1, "stop", {
    sourceFallbackRecovery: recovery,
  });
  assert.strictEqual(settled, false);
  const secondSettle = sandbox.settleSourceFallbackTerminal(0, 1, "duplicate", {
    sourceFallbackRecovery: recovery,
  });
  ...
  assert.strictEqual(
    childCalls,
    0,
    "late search completion must not start playback",
  );
...
async function testDeadlineAndManualSupersession() {
  ...
  assert.strictEqual(
    recovery.terminal,
    true,
    "expired recovery must settle once",
  );
  ...
  assert.strictEqual(
    sandbox.beginSourceFallbackPlaybackInvocation({ manual: true }),
    true,
  );
  assert.strictEqual(
    superseded.cancelled,
    true,
    "manual root playback must cancel the old transaction",
  );
  assert.strictEqual(
    sandbox.settleSourceFallbackTerminal(0, 2, "stale", {
      sourceFallbackRecovery: superseded,
    }),
    false,
  );
  assert.notStrictEqual(
    sandbox.audio.src,
    "",
    "a cancelled late transaction must not clear the new root media",
  );
}
...
function testStaticRecoveryWiring() {
  ...
  assert(
    /completeSourceFallbackRecovery\(sourceFallbackRecoveryFromOptions\(opts\)\)/.test(
      startText,
    ),
  );
  assert(
    /catchRecovery \? sourceFallbackRecoveryFailureOptions\(opts\)/.test(
      startText,
    ),
  );
  assert(
    /setupRecovery \? sourceFallbackRecoveryFailureOptions\(opts\)/.test(
      startText,
    ),
  );
  ...
  assert(
    /ownerQueueItemKey:\s*String\(audioEl\.__mineradioQueueItemKey/.test(
      progressText,
    ),
  );
  assert(/function playbackStallRecoveryOwnerStillCurrent/.test(controlsText));
  assert(
    (controlsText.match(/playbackStallRecoveryOwnerStillCurrent\(/g) || [])
      .length >= 4,
  );
  assert(
    /recoverySerial !== playbackResumeRecovery\.serial/.test(controlsText),
  );
  assert(
    /clearPlaybackResumeWatchdogs\(\);\s*playbackResumeRecovery\.serial =/.test(
      controlsText,
    ),
  );
  assert(
    /\['play', 'playing', 'pause'[\s\S]{0,500}audioEl !== audio/.test(
      progressText,
    ),
  );
  assert(
    /\['error', 'stalled'\][\s\S]{0,700}schedulePlaybackStallRecovery/.test(
      progressText,
    ),
  );
  assert(
    (startText.match(/else setTimeout\(nextTrack, 0\)/g) || []).length >= 2,
    "normal ended playback must still advance",
  );
  ...
  assert(
    !/sourceFallbackRecovery/.test(nextTrackBlock),
    "natural ended next starts a fresh playback root",
  );
}
