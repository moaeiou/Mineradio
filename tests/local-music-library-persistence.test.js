...
test("FLAC tags, embedded cover and same-name LRC survive a full library reload", async (t) => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "mineradio-local-library-"),
  );
  ...
  assert.match(
    imported.tracks[0].cover,
    /^mineradio-local:\/\/cover\/[a-f0-9]{24}/,
  );
  assert.match(
    imported.tracks[0].localUrl,
    /^mineradio-local:\/\/audio\/[a-f0-9]{24}/,
  );
  ...
  assert.match(
    library.lyricForTrack(imported.tracks[0].localFileId).lyric,
    /\[00:02\.500\]第二句/,
  );
  ...
  const restored = new LocalMusicLibrary({
    userDataPath: path.join(root, "profile"),
    parseMetadata: async () => {
      throw new Error("reload must use the persisted manifest");
    },
  }).listTracksSync();
  ...
  const audioResponse = await library.mediaResponse(
    new Request(restored.tracks[0].localUrl, {
  ...
  assert.equal(
    audioResponse.headers.get("access-control-allow-origin"),
    "http://127.0.0.1:31381",
  );
  assert.equal(Buffer.from(await audioResponse.arrayBuffer()).length, 8);

  const coverResponse = await library.mediaResponse(
    new Request(restored.tracks[0].cover),
  );
  ...
  const manifest = JSON.parse(
    fs.readFileSync(
      path.join(root, "profile", "local-music-library.json"),
      "utf8",
    ),
  );
  ...
  const unauthorizedResponse = await library.mediaResponse(
    new Request(unauthorizedUrl),
  );
...
test("subsequent imports append to the persistent library by default", async (t) => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "mineradio-local-library-append-"),
  );
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  ...
  assert.deepEqual(
    appended.tracks.map((track) => track.name),
    ["First", "Second"],
  );
  assert.equal(
    (await library.importFiles([{ path: "\\\\server\\share\\Blocked.flac" }]))
      .error,
    "NO_SUPPORTED_LOCAL_AUDIO",
  );
});
...
test("metadata and manifest failures preserve the last committed cover and index", async (t) => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "mineradio-local-library-transaction-"),
  );
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  ...
  const library = new LocalMusicLibrary({
    ...
    parseMetadata: async () => ({
      common: {
        title: "Committed",
        picture: [{ format: "image/png", data: ONE_PIXEL_PNG }],
      },
      format: { duration: 1 },
    }),
  });
  ...
  const committedManifest = fs.readFileSync(
    path.join(profile, "local-music-library.json"),
    "utf8",
  );
  ...
  library.parseMetadata = async () => ({
    common: {
      title: "Uncommitted",
      picture: [{ format: "image/png", data: secondPng }],
    },
    format: { duration: 2 },
  });
  ...
  await assert.rejects(
    () => library.importFiles([{ path: audioPath }]),
    /disk full/,
  );
  assert.equal(fs.existsSync(oldCoverPath), true);
  assert.deepEqual(fs.readFileSync(oldCoverPath), oldCover);
  assert.equal(
    fs.readFileSync(path.join(profile, "local-music-library.json"), "utf8"),
    committedManifest,
  );
  assert.equal(
    library.records.get(committed.tracks[0].localFileId).coverPath,
    oldCoverPath,
  );
  assert.equal(
    fs
      .readdirSync(path.dirname(oldCoverPath))
      .some((name) => name.endsWith(".stage")),
    false,
  );
});
...
test("embedded cover budget rejects oversized pixel dimensions on low-spec devices", () => {
  ...
  assert.equal(
    coverWithinBudget(Buffer.alloc(1024 * 1024 + 1), "image/webp"),
    false,
  );
});
...
test("renderer and Electron wiring restore persistent tracks instead of blob-only missing records", () => {
  ...
  const playerControls = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
  ...
  const coreState = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
  ...
  const homeLocal = fs.readFileSync(
    path.join(
      appRoot,
      "public",
      "js",
  ...
  assert.match(
    main,
    /new LocalMusicLibrary\(\{ userDataPath: STABLE_USER_DATA_PATH \}\)/,
  );
  ...
  assert.doesNotMatch(
    main,
    /localMusicLibrary\.importFiles\(payload && payload\.files/,
  );
  ...
  assert.match(
    startup,
    /Promise\.all\([\s\S]*persistedLocalLibraryRestorePromise/,
  );
