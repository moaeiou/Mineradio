...
async function run() {
  ...
  assert.strictEqual(
    prevented,
    0,
    "IME candidate Enter must remain available to composition",
  );
  ...
  assert.strictEqual(
    prevented,
    1,
    "plain Enter should still submit the four-character answer",
  );
  ...
  assert.strictEqual(
    harness.stats().inputFocusCount,
    1,
    "focused window should synchronously focus the hidden input once",
  );
  assert.strictEqual(harness.stats().selectionCount, 1);
  assert.strictEqual(
    harness.stats().nativeFocusRequests,
    0,
    "normal input clicks must not rebuild native TSF focus",
  );
  scheduledIds.forEach((id) =>
    assert(
      clearedTimers.has(id),
      `retry timer ${id} should be cancelled after focus`,
    ),
  );
  ...
  assert.strictEqual(
    harness.stats().inputFocusCount,
    1,
    "pointerdown and click must not refocus an already-active IME input",
  );
  assert.strictEqual(harness.stats().nativeFocusRequests, 0);

  documentStub.activeElement = null;
  harness.sandbox.__loginImeTest.schedule();
  const secondBatch = Array.from(timers.keys()).filter(
    (id) => !scheduledIds.includes(id),
  );
  inputListeners.compositionstart();
  secondBatch.forEach((id) =>
    assert(clearedTimers.has(id), `composition must cancel retry timer ${id}`),
  );
  ...
  assert.strictEqual(
    harness.stats().inputFocusCount,
    focusBeforeCancelledCallbacks,
  );

  console.log(
    "[OK] Login easter egg IME focus, retry cancellation, and composition Enter verified.",
  );
}
