...
function testRendererGoneDelayedRecovery() {
  ...
  assert.match(
    recoveryBlock,
    /mainWindowRendererRecoveryPromise/,
    "同一时刻只能有一个恢复任务",
  );
  assert.match(
    recoveryBlock,
    /reserveMainWindowRendererRecoveryAttempt\(\)/,
    "恢复必须有循环上限",
  );
  assert.match(
    recoveryBlock,
  ...
  assert.match(
    ...
    "渲染退出必须合并跟踪 WE 与桌面模式清理",
  );
  assert.match(
    goneBlock,
    /setTimeout\(\(\) => recoverMainWindowAfterRendererGone\(win, details, cleanupPromise\), 0\)/,
    "不得在 render-process-gone 回调栈内同步导航",
  );
  assert.match(
    goneBlock,
    /details\.reason[\s\S]{0,120}clean-exit/,
    "正常退出不得触发崩溃恢复",
  );
}
...
function testFullscreenVisibilityAndSystemWakeGuards() {
  ...
  assert.match(
    visibilityBlock,
    /fullDesktopModeRuntime\.getStatus\('fullscreen-visibility-guard'\)\.enabled === true/,
    "桌面模式不得被普通全屏守护拉回顶层",
  );
  assert.match(
    visibilityBlock,
  ...
  assert.match(
    visibilityBlock,
    /setInterval\([\s\S]{0,160}restoreUnexpectedFullscreenVisibility\(win, 'fullscreen-watchdog'\)/,
    "全屏可见性必须有低频运行期守护",
  );

  assert.match(
    mainText,
    /win\.__mineradioIntentionalHide = true;[\s\S]{0,180}win\.hide\(\)/,
    "托盘隐藏必须先标记为 intentional",
  );
  assert.match(
    mainText,
    /win\.on\('show'[\s\S]{0,120}win\.__mineradioIntentionalHide = false/,
    "重新显示后必须解除 intentional hide",
  );
  assert.match(
    mainText,
    /win\.on\('enter-full-screen'[\s\S]{0,220}startMainWindowFullscreenVisibilityGuard\(win\)/,
    "进入全屏必须启动守护",
  );
  assert.match(
    mainText,
    /win\.on\('leave-full-screen'[\s\S]{0,180}clearMainWindowFullscreenVisibilityGuard\(\)/,
    "退出全屏必须停止守护",
  );
  assert.match(
    mainText,
    /const\s+\{[^}]*\bpowerMonitor\b[^}]*\}\s*=\s*require\('electron'\)/,
    "主进程必须引入 powerMonitor",
  );
  assert.match(
    mainText,
    /powerMonitor\.on\('resume',[\s\S]{0,120}restoreUnexpectedFullscreenVisibility\(mainWindow, 'system-resume'\)/,
    "系统唤醒后必须检查全屏可见性",
  );
  assert.match(
    mainText,
    /powerMonitor\.on\('unlock-screen',[\s\S]{0,140}restoreUnexpectedFullscreenVisibility\(mainWindow, 'screen-unlock'\)/,
    "解锁屏幕后必须检查全屏可见性",
  );
}
...
console.log(
  "[OK] Main-window runtime recovery preserves elevated WE broker launch and restores unexpected fullscreen loss.",
);
