...
test("Qishui Passport QR bridge persists a confirmed official web session and clears it", async (t) => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "mineradio-qishui-passport-"),
  );
    ...
    async getQrCode() {
      return {
        ...
        data: {
          ...
          qrcode_index_url:
            "https://api.qishui.com/passport/web/get_qrcode/?token=official-passport-token",
        },
      };
    },
...
test("Qishui login product surface uses only the signed Passport QR flow", () => {
  ...
  assert.doesNotMatch(
    ui,
    /读取本机汽水|本机会话|Token 导入|submitQishuiTokenLogin/,
  );
