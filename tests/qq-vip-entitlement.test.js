...
function testStrictMembershipNormalization() {
  ...
  assert.strictEqual(
    unknown.resolved,
    false,
    "empty response must not become an ordinary account",
  );
  ...
  assert.strictEqual(
    textOnly.decision,
    "unknown",
    "VIP-looking labels alone must not promote an account",
  );
  ...
  assert.strictEqual(
    active.isVip,
    true,
    "unexpired official VIP evidence must be retained",
  );
  assert(
...
async function testAllProbeAggregationAndNegativeQuorum() {
  ...
  let calls = 0;
  const resolved = await qqVip.resolveQQVipFromProbes(
    probes,
  ...
  assert.strictEqual(
    resolved.isVip,
    true,
    "a later explicit positive must win over an earlier ordinary response",
  );
  ...
  assert.strictEqual(
    defaultZeroShell.decision,
    "unknown",
    "failed/default zero shell must not downgrade membership",
  );
  ...
  const unattributedSingleNegative = await qqVip.resolveQQVipFromProbes(
    ...
    async () => ({
      code: 0,
      req_1: { code: 0, data: { vip_info: { is_vip: false, vip_type: 0 } } },
    }),
  );
  assert.strictEqual(
  ...
  const matchedPositiveWithTimeouts = await qqVip.resolveQQVipFromProbes(
    ...
    async () => {
      mixedCalls += 1;
      if (mixedCalls === 1)
        return activeVipPayload(uin, Date.now() + 60 * 60 * 1000);
      if (mixedCalls === 2)
        return { code: 0, req_1: { code: 0, data: { vip: 0 } } };
      throw new Error("timeout");
    },
  );
  assert.strictEqual(
    matchedPositiveWithTimeouts.isVip,
    true,
    "one current-UIN positive must survive zero shells and timeouts",
  );
...
function testSessionScopedCacheFingerprintAndExpiry() {
  ...
  assert.strictEqual(
    keyA,
    qqVip.qqVipSessionCacheKey("10003", "ticket-A", cookieA),
  );
  assert.notStrictEqual(
    keyA,
    qqVip.qqVipSessionCacheKey("10003", "ticket-B", cookieB),
  );
  assert.notStrictEqual(
    keyA,
    qqVip.qqVipSessionCacheKey("10004", "ticket-A", cookieA),
  );
  assert(
    !keyA.includes("ticket-A"),
    "cache key must not expose a raw QQ ticket",
  );
  ...
  assert(
    ttl > 0 && ttl <= 2500,
    "positive cache must not outlive entitlement expiry",
  );
  assert.strictEqual(
...
async function testTransientFrontendFailureKeepsLastKnownGood() {
  ...
  assert.strictEqual(
    status.loggedIn,
    true,
    "transient refresh failure must keep the current account",
  );
  assert.strictEqual(
    status.isVip,
    true,
    "transient refresh failure must keep last-known verified VIP",
  );
  assert.strictEqual(status.membershipKnown, true);
  assert.strictEqual(
    status.stale,
    true,
    "preserved status must be marked stale",
  );
...
function testDesktopReauthCookieSelectionAndBudgets() {
  ...
  const loginSource = fs.readFileSync(
    path.join(ROOT, "public/js/modules/08-account/03-login-modal-flows.js"),
    "utf8",
  );
  const serverSource = fs.readFileSync(path.join(ROOT, "server.js"), "utf8");
  const playbackSource = fs.readFileSync(
    path.join(ROOT, "public/js/modules/05-playback/13-playback-start-audio.js"),
    "utf8",
  );
  ...
  const cookieEnd = mainSource.indexOf(
    "\nasync function readQQLoginCookieHeader",
    cookieStart,
  );
  const cookieContext = {
    Date,
  ...
  assert(
    header.includes("uin=preferred-y"),
    "QQ cookie selection must prefer the official y.qq.com scope",
  );
  assert(
    header.includes("qm_keyst=valid-key"),
    "QQ cookie selection must retain a valid playback key",
  );
  assert(
    !header.includes("expired-key"),
    "expired QQ cookies must be excluded",
  );
  ...
  const vkeyBudget = Number(
    (serverSource.match(/QQ_VKEY_REQUEST_TIMEOUT_MS\s*=\s*(\d+)/) || [])[1],
  );
  const audioProbeBudget = Number(
    (serverSource.match(/QQ_AUDIO_PROBE_TOTAL_MS\s*=\s*(\d+)/) || [])[1],
  );
  assert(
  ...
  assert(
    !/writeQQPlaybackVipEvidence\(Object\.assign\(\{\}, merged/.test(
      accountSource,
    ),
  );

  const fallbackSource = fs.readFileSync(
    path.join(ROOT, "public/js/modules/05-playback/11-provider-fallback.js"),
    "utf8",
  );
  assert(
    /membershipUnknown[\s\S]{0,700}!membershipUnknown/.test(fallbackSource),
  );
  assert(
    /会员待同步/.test(fallbackSource),
    "unknown QQ membership must not be rendered as an ordinary account",
  );
  assert(
    /membershipUnknown[\s\S]{0,500}vipSyncState:\s*authIncomplete/.test(
      serverSource,
    ),
  );
  assert(
...
function testPackagingIncludesQQVipModule() {
  ...
  assert(
    files.includes("*-api.js"),
    "Electron package must include provider API modules",
  );
