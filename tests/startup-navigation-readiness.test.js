  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      const error = new Error(
        `${label || "startup operation"} timed out in test`,
      );
...
async function testTrustedReadySignalRequiresTheExpectedDocument() {
  ...
  webContents.emit(
    "did-navigate",
    {},
    "http://127.0.0.1:31381/?startupAttempt=2&startupAt=2",
    200,
  );
  assert.strictEqual(
    signal.isReady(),
    false,
    "a different trusted navigation must not satisfy this attempt",
  );
  ...
  assert.deepStrictEqual(
    {
      url: win.__mineradioTrustedMainDocumentReady.url,
      phase: win.__mineradioTrustedMainDocumentReady.phase,
    },
...
async function testStalledLoadPromiseAcceptsTrustedDomReady() {
  const harness = createLoadWithRetryHarness({
  ...
  assert.match(
    result,
    /^http:\/\/127\.0\.0\.1:31381\/\?startupAttempt=1&startupAt=\d+$/,
  );
  assert.strictEqual(
    fake.loadCalls(),
    1,
    "trusted DOM readiness must finish on the first navigation attempt",
  );
  assert.strictEqual(
    fake.stopCalls(),
    0,
    "a visible trusted document must never be stopped because loadURL stayed pending",
  );
  assert(
    harness.states.some(
      (entry) =>
        entry.phase === "navigation-ready" &&
        entry.details.navigationReadyPhase === "dom-ready",
    ),
...
async function testTrulyUnreadyDocumentStillRetriesAndFails() {
  ...
  assert.strictEqual(
    fake.loadCalls(),
    2,
    "a document with no trusted ready signal must use the bounded retry",
  );
  assert(
    fake.stopCalls() >= 2,
    "only genuinely unready attempts may be stopped",
  );
  assert.strictEqual(
...
function testSourceContractKeepsTimeoutFailOpenNarrow() {
  ...
  assert.match(
    loadWithRetrySource,
    /if \(readySignal\.isReady\(\)\) return;[\s\S]{0,100}win\.webContents\.stop\(\)/,
  );
  assert.match(
    loadWithRetrySource,
    /MINERADIO_STARTUP_TIMEOUT[\s\S]{0,100}readySignal\.isReady\(\)/,
  );
  assert.match(
    loadWithRetrySource,
    /finally \{[\s\S]{0,100}readySignal\.cancel\(\)/,
  );
  assert.match(
    loadWithRetrySource,
    /MINERADIO_STARTUP_TEST_STALL_LOAD_PROMISE/,
  );
}
...
async function main() {
  ...
  console.log(
    "[OK] startup navigation accepts a trusted ready document without killing it, while truly unready loads still retry and fail",
  );
}
