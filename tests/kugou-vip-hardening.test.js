...
function testCookieVipIsOnlyAPendingHint() {
  ...
  assert.strictEqual(
    membership.isVip,
    false,
    "cookie VIP cannot become an account VIP without official verification",
  );
  assert.strictEqual(
    membership.membershipKnown,
    false,
    "cookie fields cannot finalize account membership",
  );
  assert.strictEqual(
    membership.membershipHintLevel,
    "vip",
    "the UI may retain a pending VIP hint",
  );
  assert.strictEqual(
    membership.membershipVerified,
    false,
    "cookie VIP is not official verification",
  );
  assert.strictEqual(
    rights.level,
    "none",
    "unverified hints must not render as confirmed VIP",
  );
  assert.strictEqual(
    rights.canPlayVipTracks,
    false,
    "unverified cookie VIP cannot unlock VIP playback",
  );
  assert.strictEqual(
    rights.canPlaySvipTracks,
    false,
    "unverified cookie VIP cannot unlock SVIP playback",
  );
  assert.strictEqual(
    rights.maxQuality,
    "standard",
    "unverified cookie VIP cannot unlock paid quality",
  );
  assert.strictEqual(
    kugouMembershipRights({
...
function testBatchResponsesStayInsideTheRequestedAccount() {
  ...
  assert.strictEqual(
    membership.isVip,
    false,
    "another map-keyed account must not promote the requested account",
  );
...
function testWebRoleInfoIsSourceScopedAndExpiryAware() {
  ...
  [
    ...
    assert.strictEqual(
      membership.membershipKnown,
      true,
      `web role ${role} must be authoritative`,
    );
    assert.strictEqual(
      membership.membershipVerified,
      true,
      `web role ${role} must be verified`,
    );
    assert.strictEqual(
      membership.vipLevel,
      level,
      `web role ${role} must map to ${level}`,
    );
    assert.strictEqual(
      membership.isVip,
      true,
      `web role ${role} must expose VIP identity`,
    );
    assert.strictEqual(
      membership.isSvip,
      level === "svip",
  ...
  [31, 33].forEach((role) => {
    ...
    assert.strictEqual(
      membership.membershipKnown,
      true,
      `music-package role ${role} must be known`,
    );
    assert.strictEqual(
      membership.isVip,
      false,
      `music-package role ${role} must not become Kugou VIP`,
    );
    assert.strictEqual(
      membership.isSvip,
      false,
      `music-package role ${role} must not become Kugou SVIP`,
    );
    assert.strictEqual(
      membership.vipLevel,
      "none",
      `music-package role ${role} must keep a non-VIP badge`,
    );
    assert.strictEqual(
  ...
  const ordinary = normalizeKugouVipPayloadV2(
    webPayload({
      role: 0,
      producttype: 0,
      usertype: 0,
  ...
  assert.strictEqual(
    ordinary.membershipKnown,
    true,
    "an explicit web ordinary response must be known",
  );
  assert.strictEqual(
    ordinary.isVip,
    false,
    "an explicit web ordinary response must stay ordinary",
  );

  const expired = normalizeKugouVipPayloadV2(
    webPayload({
      role: 1,
      producttype: 1,
  ...
  assert.strictEqual(
    expired.membershipKnown,
    true,
    "an expired web role remains an authoritative result",
  );
  assert.strictEqual(
    expired.isVip,
    false,
    "an expired rawVipEndTime must not authorize VIP",
  );
  assert.strictEqual(
  ...
  const invalidExpiry = normalizeKugouVipPayloadV2(
    webPayload({
      role: 1,
  ...
  assert.strictEqual(
    invalidExpiry.membershipKnown,
    true,
    "an invalid expiry remains an authoritative response",
  );
  assert.strictEqual(
    invalidExpiry.isVip,
    false,
    "an invalid explicit expiry must fail closed",
  );

  const timestampExpiry = normalizeKugouVipPayloadV2(
    webPayload({
      role: 2,
  ...
  assert.strictEqual(
    timestampExpiry.isVip,
    true,
    "a future Unix timestamp must remain supported",
  );
  assert.strictEqual(
    timestampExpiry.expiresAt,
    4102444800000,
    "Unix seconds must normalize to milliseconds",
  );
  assert.strictEqual(
    kugouMembershipTime("2099-12-31"),
  ...
  assert.strictEqual(
    normalizeKugouVipPayloadV2(otherAccount, { userid: "999001" })
      .membershipKnown,
    false,
  ...
  assert.strictEqual(
    untrustedRole.isVip,
    false,
    "an untrusted role field must not promote the account",
  );
}
...
function testRecentOfficialPositiveSurvivesOnlyUnknownProbes() {
  ...
  const stale = stabilizeKugouVipProbe(
    key,
    { __kugouMembershipUnknown: true },
    auth,
    now + 1000,
  ...
  assert.strictEqual(
    staleMembership.vipLevel,
    "vip",
    "stale official identity may retain the VIP badge",
  );
  assert.strictEqual(
    staleMembership.membershipVerified,
    false,
    "stale identity is no longer current verification",
  );
  assert.strictEqual(
  ...
  const ordinary = stabilizeKugouVipProbe(
    key,
    {
      data: {
        userid: "9010",
  ...
  const afterOrdinary = stabilizeKugouVipProbe(
    key,
    { __kugouMembershipUnknown: true },
    auth,
    now + 3000,
  ...
  assert.strictEqual(
    kugouMembershipRights(normalizeKugouVipPayloadV2(afterOrdinary, auth))
      .canPlayVipTracks,
    false,
...
function testStaleSvipNeverAuthorizesAndMissingExpiryIsNotRetained() {
  ...
  assert.strictEqual(
    staleSvip.vipLevel,
    "svip",
    "stale official identity may retain the SVIP badge",
  );
  assert.strictEqual(
    staleSvipRights.canPlayVipTracks,
    false,
    "stale SVIP cannot authorize VIP playback",
  );
  assert.strictEqual(
    staleSvipRights.canPlaySvipTracks,
    false,
    "stale SVIP cannot authorize SVIP playback",
  );
  assert.strictEqual(
    staleSvipRights.maxQuality,
    "standard",
    "stale SVIP cannot authorize Hi-Res quality",
  );
  ...
  assert.strictEqual(
    noExpiryMembership.vipLevel,
    "vip",
    "fresh official VIP evidence remains visible without an expiry field",
  );
  assert.strictEqual(
    kugouMembershipRights(
      Object.assign({}, noExpiryMembership, {
        playbackReady: true,
      }),
    ).canPlayVipTracks,
    true,
...
console.log(
  "[OK] Kugou VIP hardening: web roles are source-scoped, expiry-aware, and cannot overgrant music-package accounts.",
);
